...
public interface MessageMapper {
    ...
    @Select("SELECT emotion FROM message WHERE roomId = #{roomId} AND type = #{userType} ORDER BY time ASC LIMIT 1;")
    public String getLastEmotion(int roomId, String userType);

    @Select("SELECT COUNT(*) FROM message WHERE roomId = #{roomId} AND type = 'client' AND emotion = 'angry';")
